...
def _build_http_app(mcp, settings):
    ...
    async def health(_: Request) -> JSONResponse:
        # Pure local liveness probe — must not depend on the Proofpoint APIs
        # being reachable, and must return exactly {"status": "ok"} (no extra
        # fields) per the Vendor MCP SOP's health-endpoint contract.
...
def main() -> None:
    ...
    print(
        f"Proofpoint MCP server listening on http://{settings.mcp_http_host}:{settings.mcp_http_port}/mcp",
        file=sys.stderr,
    )
