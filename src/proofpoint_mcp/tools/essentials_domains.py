...
def register(mcp: FastMCP, client_factory: Callable[[], ProofpointEssentialsClient | None]) -> None:
    ...
    @mcp.tool()
    async def proofpoint_essentials_domains_delete_domain(domain: str, target_domain: str) -> str:
        """Delete a Domain.

        API: DELETE /orgs/{domain}/domains/{targetDomain}

        Args:
            domain: Required. Any Domain associated with the Organization
            target_domain: Required. Name of the domain to be deleted
        """
    ...
    @mcp.tool()
    async def proofpoint_essentials_domains_get_health(domain: str, domaintobediagnosed: str) -> str:
        """Run a health check on the domain.

        API: GET /orgs/{domain}/domains/{domaintobediagnosed}/health

        Args:
            domain: Required. Any Domain associated with the Organization
            domaintobediagnosed: Required. Name of the Domain to be diagnosed
        """
        client = client_factory()
        if client is None:
            return NO_ESSENTIALS_TOKEN
        path = f"/orgs/{domain}/domains/{domaintobediagnosed}/health"
        params = {}
        try:
            result = await client.get(path, params=params)
            return json.dumps(result, indent=2, default=str)
    ...
    @mcp.tool()
    async def proofpoint_essentials_domains_put_domain(domain: str, target_domain: str, body: dict) -> str:
        """Update a Domain associated with an Organization.

        API: PUT /orgs/{domain}/domains/{targetDomain}

        Args:
            domain: Required. Any Domain associated with the Organization
            target_domain: Required. Name of the domain to be updated
            body: Required. Domain data
        """
        client = client_factory()
        if client is None:
