...
def register(mcp: FastMCP, client_factory: Callable[[], ProofpointTapClient | None]) -> None:
    ...
    @mcp.tool()
    async def proofpoint_tap_people_get_top_clickers(window: str, size: str | None = None, page: str | None = None) -> str:
        """Fetch the identities and attack index of the top clickers within your organization for a given period.

        API: GET /v2/people/top-clickers

        Args:
            window: Required. Days of data to retrieve. Accepted values: 14, 30, or 90 (per official People API doc).
            size: Optional. Max results to return, ordered by attackIndex (default: 100, max: 200)
        ...
        try:
            result = await client.get("/v2/people/top-clickers", params=params)
